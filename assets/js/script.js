function adicionarTec(){
    ...
    if(val) {
        var ul = $("ul#tec");
        var html = "<li class=\"list-group-item\"><span>" + val + "</span><i class='fa fa-times'></i></ul>";
...
function pegarValorTEC(){
    ...
    return $("ul#tec li:eq(" + x + ") span").text();
}
...
function pegarValorTS(){
    ...
    var MAX = ulTecLi.length;
    var x = Math.floor((Math.random() * MAX)) - 1;
    return $("ul#ts li:eq(" + x + ") span").text();
}
...
$(document).on("click", ".entrada .panel .list-group .list-group-item i", function() {
